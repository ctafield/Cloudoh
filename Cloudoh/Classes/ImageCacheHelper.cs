            ...
            lock (IsProfileImageCachedLock)
            {
                ...
                try
                {
                    ...
                    using (var myStore = IsolatedStorageFile.GetUserStoreForApplication())
                    {
                        ...
                        var targetFile = currentImage;
                        
                        if (!targetFile.StartsWith(userFolder))                            
                            targetFile = Path.Combine(userFolder, currentImage);
        ...
        public static long GetSizeOfCachedImages(string folderName)
        {
            ...
            using (var myStore = IsolatedStorageFile.GetUserStoreForApplication())
            {
                ...
                if (folderName != ApplicationConstants.UserCacheStorageFolder)
                {
                    ...
                    foreach (var file in theseFiles.ToList())
                    {
                ...
                {
                    // errr, dunno. Lets not error though eh.
                }
            }
